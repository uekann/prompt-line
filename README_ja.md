...
Prompt LineはVimキーバインドに慣れているユーザー向けに、vimライクなモーダル編集をサポートしています。有効にすると、ウィンドウ右上にモードインジケーターが表示されます。

**Vimモードを有効にする：**
1. 設定ファイルを編集（場所は上記参照）
2. `vim.enabled: true` に設定
3. Prompt Lineを再起動

**サポートされるモード：**
- **ノーマルモード**（デフォルト）：テキストの移動と操作
- **インサートモード**：通常のテキスト入力
- **ビジュアルモード**：テキスト範囲の選択と操作
- **ビジュアルラインモード**：行全体の選択と操作
...
- その他のキーは通常のテキスト入力として動作

*ビジュアル/ビジュアルラインモード:*
- `h/j/k/l`: 選択範囲を拡張
- `y`: 選択範囲をコピーしてノーマルモードに戻る
- `d`: 選択範囲を削除（カット）してノーマルモードに戻る
- `p`: 選択範囲をコピーしたテキストで置換
- `Esc` または `Ctrl+[`: 選択をキャンセルしてノーマルモードに戻る

**モードインジケーターの色：**
- 🔵 **青色**: ノーマルモード
- 🟢 **緑色**: インサートモード
- 🟣 **紫色**: ビジュアル/ビジュアルラインモード
